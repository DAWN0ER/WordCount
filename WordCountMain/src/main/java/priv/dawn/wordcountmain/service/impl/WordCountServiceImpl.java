...
public class WordCountServiceImpl implements WordCountService {
    ...
    // TODO: 2024/5/3 可能需要添加分布式锁防止缓存穿透
    @Override
    ...
    // TODO: 2024/5/4 后续添加Redis来缓存
    @Override
    public List<String> getWordCounts(int fileUID) {
        String state = (String) progressRedisProxy.opsForValue().get(fileUID);
        if (!ALREADY_FINISHED.equals(state)) {
            if (this.getProgress(fileUID) < 100f) return null;
        }
        return workerDubboCilent.getWordCountList(fileUID);
    }
}
